...
// 非流式响应无 usage 字段（SPEC §1.4 实测），不得假设其存在。
...
func Aggregate(r io.Reader) (map[string]any, error) {
	...
	if first, err := br.Peek(1); err == nil && first[0] == '{' {
		...
		return normalizeNonStream(out), nil
	}
	var (
		id, model     string
		created       float64
	...
	// 注意：SPEC §1.4 实测非流式响应无 usage 字段，这里不假设、不补造。
	return resp, nil
}
...
// 超时返回后调用方关闭连接即可回收资源。
func AggregateCtx(ctx context.Context, r io.Reader) (map[string]any, error) {
	type res struct {
		resp map[string]any
		err  error
	}
